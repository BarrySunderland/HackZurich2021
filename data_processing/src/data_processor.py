...
class DataProcessor:
    ...
    def combine_events(save:bool=True):
        ...
        """
        # RSSI data is too big to load onn cpu at a time.
        column_of_interest = ["PositionNoLeap","Date", "Latitude","Longitude","A1_TotalTel","A1_ValidTel","A2_RSSI","A2_TotalTel","A2_ValidTel"]
        rssi_df_s = pd.read_csv(
            PATHS.rssi,
            chunksize=10000,
            iterator=True,
        ...
        with tqdm.tqdm(total=3098) as pbar:
            for rssi_df in rssi_df_s:
                ...
                merged_dfs.append(rssi_df[column_of_interest].groupby(by=["Date", "PositionNoLeap"]).mean())
                pbar.update(1)
        result_df = pd.concat(merged_dfs)
        if save:
            result_df.to_csv(DataProcessor.gen_proc_file_name(PATHS.rssi))
        return pd.concat(merged_dfs)
    ...
    def make_positional_mapping(save=True):
        ...
        with tqdm.tqdm(total=3098) as pbar:
            for rssi_df in rssi_df_s:
                temp =rssi_df[column_of_interest].groupby(by=["PositionNoLeap"]).mean().reset_index()
    ...
    #             break
        result_df = pd.concat(merged_dfs)
        result_df =  result_df.reset_index().drop_duplicates('Position_m').set_index('Position_m')
        if save:
            result_df.to_csv(PATHS.data+"/location.csv")
        return result_df
